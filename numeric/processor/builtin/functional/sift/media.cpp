...
/*
	...
	[1]	(!is_unsigned && min < x < 0)				->	signed_degree(|x|)
	[1]	(!is_unsigned && x > 0)					->	signed_degree
...
class degree
{
	...
	static constexpr size_type signed_degree = semiotic::template signed_degree<0, (x < 0 ? -x : x)>::value;
