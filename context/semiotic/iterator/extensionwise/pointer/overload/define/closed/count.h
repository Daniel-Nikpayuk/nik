...
static rtn label##_return(size_type & closed_count, WPointer out, EWPointer end) \
{ \
	while (out != end) \
	{ \
		*out=new Node(); \
...
template<typename Node, typename WNode, typename WPointer, typename EWPointer> \
static rtn label##_return(size_type & closed_count, WPointer out, EWPointer end, size_type in) \
{ \
	while (out != end) \
	{ \
		*out=new Node[in]; \
...
static rtn label##_return(size_type & closed_count, WPointer out, RIterator in, ERIterator end) \
{ \
	while (in != end) \
	{ \
		lp(*out)op(*in)rp; \
...
static rtn label##_return(size_type & closed_count, WPointer out, RPointer in, ERPointer end) \
{ \
	while (in != end) \
	{ \
		*out=new Node[*in]; \
...
static rtn label##_return(size_type & closed_count, WPointer out, RIterator1 in1, RIterator2 in2, ERIterator end2) \
{ \
	while (in2 != end2) \
	{ \
		(*out)=(*in1)op(*in2); \
