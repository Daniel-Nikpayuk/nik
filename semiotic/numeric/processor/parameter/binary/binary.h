...
#ifndef NIK_SEMIOTIC_NUMERIC_PROCESSOR_PARAMETER_BINARY_H
#define NIK_SEMIOTIC_NUMERIC_PROCESSOR_PARAMETER_BINARY_H
...
*/
		...
		struct lower_half
			{ enum : size_type { value = low<x, snp_policy::unit::half::length>::value }; };
		template<size_type x>
		struct upper_half
			{ enum : size_type { value = high<x, snp_policy::unit::half::length>::value }; };
		...
		struct order
		{
			...
			struct fast_order
			{
				enum : size_type
				{
					value=meta::if_then_else<band<secondary, (n>>1), n>::value,
			...
			enum : size_type { value=fast_order<0, x, snp_policy::unit::length>::value };
		};
	};
    }
   }
