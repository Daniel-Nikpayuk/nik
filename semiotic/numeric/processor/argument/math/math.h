...
#ifndef NIK_SEMIOTIC_NUMERIC_PROCESSOR_ARGUMENT_MATH_H
#define NIK_SEMIOTIC_NUMERIC_PROCESSOR_ARGUMENT_MATH_H
...
*/
			struct shift
			{
				...
				{
					out=(snp_policy::unit::one & in2);
					return snp_policy::unit::max_power + (in2 >> snp_policy::unit::one);
				}
			};
...
*/
				static size_type with_return(size_type & out, size_type in1, size_type in2, size_type d)
				{
					...
					in1 <<= snp_policy::unit::half::length;
					(big_in2 <<= snp_policy::unit::half::length) += little_in2;
			...
			{
				size_type t=snp_policy::unit::zero, tc=multiply::high_return(t, v, q);
				if (tc > uc || (tc == uc && t > u))
				{
					--q;
					return true;
...
*/
			static size_type clean_return(size_type & r, size_type in1, size_type in2, size_type d)
			{
				...
				if (r || in1 >= d)
				{
					...
					q <<= snp_policy::unit::half::length;
				}

				r=binary::high(in1);
				(in1 <<= snp_policy::unit::half::length)+=in2;
				// in2 is now free.
			...
			static size_type with_return(size_type in1, size_type in2, size_type d)
			{
				size_type	rank=binary::order(d)+1,
						power=snp_policy::unit::length-rank;
				...
				return clean_return(binary::high(in1),
					(in1 <<= snp_policy::unit::half::length) += binary::high(in2),
					binary::low(in2), d);
			}
