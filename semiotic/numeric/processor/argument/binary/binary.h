...
#ifndef NIK_SEMIOTIC_NUMERIC_PROCESSOR_ARGUMENT_BINARY_H
#define NIK_SEMIOTIC_NUMERIC_PROCESSOR_ARGUMENT_BINARY_H
...
*/
	...
	// common enough to optimize:
		static size_type shift_up(size_type in)
			{ return in << snp_policy::unit::half::length; }
...
*/
		...
		static size_type low(size_type in)
			{ return (snp_policy::unit::filter::low_pass & in); }
...
*/
			...
			{
				size_type m=mid(secondary, (N >> 1), N);
				if (m) primary += (N >> 1);
				else m=mid(secondary, 0, (N >> 1));
		...
		static size_type order(size_type in)
			{ return unroll<snp_policy::unit::length>::order(0, in); }
	};
    }
   }
  }
