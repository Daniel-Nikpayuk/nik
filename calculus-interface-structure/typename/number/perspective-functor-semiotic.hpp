...
struct functor
{
	...
	#define safe_name
		...
		#include nik_typedef(calculus, perspective, typedin, functor)

	#undef safe_name

	#include nik_typedef(calculus, typedin, number, structure)
	#include nik_typedef(calculus, typedin, number, identity)
...
*/
	...
	inline static void display(const number<Values...> & b)
	{
		using is_empty = typename is_null<number<Values...>>::rtn;
		const act<register_type> a;

		Builtin::functor::display("number:");
